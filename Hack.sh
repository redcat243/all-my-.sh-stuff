    ...
    case $choice in
        1|3|4)
            ...
            if [[ "$choice" == "3" ]]; then
                ...
                ssh "$target" 'if ! command -v brew &>/dev/null; then NONINTERACTIVE=1 /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.co/Homebrew/install/HEAD/install.sh)"; fi; brew tap redcat243/catbrowser && brew install --cask redcat243/catbrowser/catbrowser'
            fi
            ;;
